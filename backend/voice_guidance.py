...
- 参考表达必须是一段可以直接用于回答该问题的完整表达：先给结论，再给 2—4 个有逻辑顺序的支撑点，并保留用户核心立场。
- 生成参考表达前，先检查 logic.improvements、episode_alignment.missing_angles、supplementary_angles 和 verification_hint：其中与回答直接相关、且不会虚构事实的内容，必须被吸收到 reference_answer 的结论或支撑点中，不能只在“补充提示”里另列。
- 参考表达不得只是 conclusion 或 points 的同义改写。若用户的回答存在可执行的表达改进或可补充角度，参考表达至少应新增一个具体的判断维度、分析框架、例外条件或行动化表达，并让它服务于回答本身。
- ai_additions 只记录已经整合进参考表达、但不来自用户原话的新增视角；不要把这些新增视角另写成一份与参考表达平行的答案。
- 不要把“如何回答”的说明混进参考表达；参考表达本身应当自然、完整、可直接朗读。
...
必须返回这个结构：
{
  ...
  "guidance": {
    ...
    "reference_answer": {
      "conclusion": "保留用户立场、同时吸收必要补充后的结论",
      "points": ["可直接朗读的完整支撑表达，包含必要的补充维度"],
      "ai_additions": ["已融入上述参考表达的AI新增视角"]
    },
